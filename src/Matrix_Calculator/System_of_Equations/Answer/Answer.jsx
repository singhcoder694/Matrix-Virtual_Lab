    ...
    return (
        <div style={{display:"flex", flexDirection:"row", justifyContent:"space-around", textAlign:"center"}}>
            {answer.map((ans,index)=>(
                <div style={{display:"flex", flexDirection:"column"}} key={index+"Index"}>
                    <p style={{fontSize:"1.5rem", color:"white", fontWeight:"900" }}>x{(index+1)}</p>
                    <input className='box' value={ans} style={{"textAlign":"center"}} readOnly></input>
                </div>
            ))}
        </div>
    )
